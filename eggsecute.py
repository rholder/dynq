...
def main(script_path, output_path):
    ...
    module_files = []
    module_files.extend(collect_module_files('dynq', ''))
    module_files.extend(collect_module_files('click', ''))

    # this is all for boto3 
    ...
    # filter out everything but .py's, .pem's, and .json's
    filtered_files = [x for x in module_files if x[1].endswith(".py") or x[1].endswith(".pem") or x[1].endswith(".json")]
